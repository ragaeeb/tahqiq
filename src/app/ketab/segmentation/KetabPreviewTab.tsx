...
import { buildGeneratedOptions } from '@/app/shamela/segmentation/JsonTab';
 ...
 */
export const KetabPreviewTab = () => {
    const { ruleConfigs, sliceAtPunctuation, tokenMappings, replacements } = useSegmentationStore();
    const pages = useKetabStore((state) => state.pages);
    ...
    const { segments, error } = useMemo(() => {
        console.log('[KetabPreviewTab] Generating segments preview');
        console.log('[KetabPreviewTab] ruleConfigs:', ruleConfigs);
        console.log('[KetabPreviewTab] pages count:', pages.length);

        if (ruleConfigs.length === 0) {
            console.log('[KetabPreviewTab] No ruleConfigs, returning empty');
            return { error: null, segments: [] };
        }

        try {
            const optionsJson = buildGeneratedOptions(ruleConfigs, sliceAtPunctuation, tokenMappings, replacements);
            console.log('[KetabPreviewTab] Generated options JSON:', optionsJson);

            const options = JSON.parse(optionsJson) as SegmentationOptions;
            console.log('[KetabPreviewTab] Parsed options:', options);

            // Convert ketab pages to segmentation format
            const segmentationPages: Page[] = pages.map((p) => ({ content: htmlToMarkdown(p.body), id: p.id }));
            console.log('[KetabPreviewTab] Converted pages for segmentation:', segmentationPages.length);

            const result = segmentPages(segmentationPages, options);
            console.log('[KetabPreviewTab] Segmentation result:', result.length, 'segments');
            return { error: null, segments: result };
        } catch (err) {
            console.error('[KetabPreviewTab] Error during segmentation:', err);
            return { error: err instanceof Error ? err.message : 'Unknown error', segments: [] };
        }
    }, [ruleConfigs, sliceAtPunctuation, tokenMappings, replacements, pages]);
        ...
        return (
            <div className="flex h-full items-center justify-center text-muted-foreground">
                ...
                <span className="text-xs">
                    (Debug: {ruleConfigs.length} rules, {pages.length} pages)
                </span>
            </div>
        );
    }
