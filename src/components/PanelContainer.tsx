...
export const PanelContainer = ({ children, onCloseClicked, title = 'Segment Pages' }: PanelContainerProps) => {
    ...
    return (
        <Dialog modal={false} open>
            <DialogContent
                className="!max-w-none !w-[60vw] fixed top-0 right-auto left-0 flex h-full translate-x-0 translate-y-0 flex-col rounded-none border-t-0 border-r border-b-0 border-l-0 p-0 [&>button]:hidden"
                ref={contentRef}
            >
